...
Modules: `zsh`, `fonts`, `ghostty`, `iterm2`, `tmux`, `nvim`, `opencode`, `npm`, `go`, `Codex`, `agents`.
...
- **Global** (`Codex` module) — `Codex/.Codex/{skills,agents}` stows to `~/.Codex/{skills,agents}`, making those skills/agents available in every project. `Codex/install.sh` pre-creates the target dirs as real directories so Stow links *individual* files/subdirs in, coexisting with any unmanaged files already in `~/.Codex/` (e.g. hand-placed agents). Use `stow -R Codex` after adding definitions.
- **Project** (this repo only) — the repo-root `.Codex/{skills,agents}` is read directly by Codex when cwd is this repo and is **not** stowed. If a name exists in both scopes, the project-level definition wins.

When adding here: skills are `skills/<name>/SKILL.md`, agents are `agents/<name>.md`, both with `name` + `description` frontmatter. See `Codex/README.md`.
